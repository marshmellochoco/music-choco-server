...
app.put('/playlists/:id/tracks', userAuth, async (req, res) => {
...
app.delete('/playlists/:id/tracks', userAuth, async (req, res) => {
...
app.put('/playlists/:id', userAuth, async (req, res) => {
    let playlist = await updatePlaylist(req.user, req.params.id, req.body.name);
    res.send(playlist);
});

app.delete('/playlists/:id', userAuth, async (req, res) => {
...
app.get('/users/:id/library/albums', async (req, res) => {
    let user = await getUserLibraryAlbum(req.params.id);
    res.send(user);
});

app.get('/users/:id/library/artists', async (req, res) => {
    let user = await getUserLibraryArtist(req.params.id);
    res.send(user);
});

app.get('/users/:id/library/playlists', async (req, res) => {
...
app.post('/login/unencrypt', async (req, res) => {
    let body = await loginUserUnencrypt(req.body.credential);
    if (!body.error) res.send(body);
    else res.sendStatus(body.error);
});
